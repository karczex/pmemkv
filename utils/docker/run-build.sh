...
function tests_gcc_debug_cpp11() {
	...
	make -j$(nproc)
	ctest -j$(nproc) -E "_memcheck|_drd|_helgrind|_pmemcheck|_pmreorder" --timeout 590 --output-on-failure
...
function tests_gcc_debug_cpp14_valgrind_other() {
	...
	make -j$(nproc)
	ctest -j$(nproc) -R "_helgrind|_pmemcheck|_pmreorder" --timeout 590 --output-on-failure
...
function tests_gcc_debug_cpp14_valgrind_memcheck_drd() {
	...
	make -j$(nproc)
	ctest -j$(nproc) -R "_memcheck|_drd" --timeout 590 --output-on-failure
